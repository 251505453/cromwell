  ...
  implicit def ct: Aux[CwlType, CommandPartBuilder] = {
    at {
      cwlType => {
        cwlType match {
          case CwlType.Null => {
            case (_, _, _, _) => CommandPartsList.empty
          }
          case _ => {
            case (None, _, _, _) => CommandPartsList.empty
            case (Some(binding), womValue, key, expressionLib) =>
              List(binding.toCommandPart(key, womValue, expressionLib))
          }
        }
      }: CommandPartBuilder
    }
